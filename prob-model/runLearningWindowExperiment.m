function runLearningWindowExperiment()
  ...
  [svm_training, svm_pred, svm_corr] = runSvmExperiment(min_N, X, d, max_N, refresh_rate);
  ...
  [svm_training_win, svm_pred_win, svm_corr_win] = runSvmExperiment(min_N, X, d, max_N, refresh_rate, win_len);
  ...
  [prob_train_full, prob_pred_full, prob_corr_full] = runProbModelExperiment(n, K, X, d, max_N, refresh_rate);
  disp('Computing quality for full probabilistic experiment')
  [prec_prob_full, rec_prob_full, F_prob_full] = computeQuality(prob_corr_full, d, n);
  ...
  [prob_train_win, prob_pred_win, prob_corr_win] = runProbModelExperiment(n, K, X, d, max_N, refresh_rate, win_len);
  disp('Computing quality for windowed probabilistic experiment')
  [prec_prob_win, rec_prob_win, F_prob_win] = computeQuality(prob_corr_full, d, n);
  % Compare the performance of the windowed and non-windowed run: duration of training, duration of prediction, and F-measure.
endfunction

function [svm_training_serial, svm_prediction_serial, svm_correctness_serial] =  runSvmExperiment(min_N, X, d, max_N, refresh_rate, win_len = Inf)
  svm_training_serial = zeros(1, max_N);
  svm_prediction_serial = zeros(1, max_N);
  svm_correctness_serial = zeros(1, max_N);
...
function [prob_train_full, prob_pred_full, prob_corr_full] = runProbModelExperiment(n, K, X, d, max_N, refresh_rate, win_len = Inf)
  win_n = max(1, n - win_len + 1);
  [D, I, N] = size(X);
  last_training = 0;
  ...
  dims = crossReduceDimensions(X(:, :, 1:n), d(1:n));
  toc()
  X_tr = X(:, :, 1:n);
  d_tr = d(1, 1:n);
  %% Baseline model
